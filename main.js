...
function aboutWindow() {
  ...
  // recebimento da mensagem do renderizador da janela sobre para fechar a janela usando o botão OK
  ipcMain.on('about-exit', () => {
    // validação (se exitir a janela e a mesma não estiver sido destruida, fechar)
...
ipcMain.on('create-note', async (event, stickyNote) => {
  ...
  //Salvar a nota no banco de dados (Passos 3: fluxo)
  newNote.save()
...
// Passo 2: Receber de renderer o pedido para listar as notas e fazer a busca de dados
ipcMain.on('list-notes',async (event) => {
  ...
  try {
    ...
    //Obs: o IPC (string) | banco (JSON) (é necessário um conversão usando JSON.strinify())
    //event.reply() resposta a uma solicitação (específica do solicitante)
